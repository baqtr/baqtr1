...
token = "6979052357:AAHrPEDS2CTGPa5XtMWv_F0vR_ZkdOkVkKM"
...
def START(message):            
    ...
    pro = types.InlineKeyboardButton(text="تقييم البوت✅", url=f"https://t.me/BBXX5BOT?start=0007rsflzu")     
    Keyy = types.InlineKeyboardMarkup()
    Keyy.row_width = 1
    Keyy.add(s1, s2, s3, s4,s5,s6,s7,s8,s9,pro)
    bot.send_message(message.chat.id, text=f"اهلا بك في بوت التشفير  قم بختيار نوع التشفير ", parse_mode="markdown", reply_markup=Keyy)
#-----------------------[encode]----------------------#    
...
def encode2(message,call):
    bot.send_message(message.chat.id, text=f" قم برسال ملف لتشفيره بصيغة [{call}]")
